...
def get_controller():
    ...
    dtheta = ctrl.Antecedent(np.arange(-1 * 100, 100, resolution[1]), 'dtheta')
    ...
    dtheta['vn'] = fuzz.trapmf(dtheta.universe, [-204.12, -204.04, -1.57, -0.589])
    ...
    dtheta['vp'] = fuzz.trapmf(dtheta.universe, [0.5973, 1.579, 204.048, 204.128])
...
#    rules.append(ctrl.Rule((theta['z'] | theta['mp']) & dtheta['vp'], force['vn']))
    rules.append(ctrl.Rule(theta['mp'] & dtheta['vp'], force['vn']))
    rules.append(ctrl.Rule(theta['z'] & dtheta['vp'], force['vn']))
